...
public class MainPageObject {
    ...
    public boolean isElementLocatedOnTheScreen(String locator){
        int element_locator_by_y = this.waitForElementPresent(locator, "Cannot find element by locator", 5).getLocation().getY();
    ...
    public void scrollWebPageTillElementNotVisible(String locator, String error_message, int max_swipes){
        if(Platform.getInstance().isMw()){
            int already_swiped = 0;
            WebElement web_element = waitForElementPresent(locator, error_message);

            while (!this.isElementLocatedOnTheScreen(locator)){
                scrollWebPageUp();
                ++already_swiped;
                if (already_swiped > max_swipes){
                    Assert.assertTrue(error_message, web_element.isDisplayed());
                }
            }
        } else {
            System.out.println("Method 'scrollWebPageTillElementNotVisible()' does nothing for platform" + Platform.getInstance().getPlatformVar());
        }
    }
